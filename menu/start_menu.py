...
class StartMenu:
    """the menu in front of the game start, including the opening, introduction, and choosing level."""
    def __init__(self):
        # win
    ...
    def menu_run(self):
        ...
        while not quit_game:
            ...
            if self.page.startswith('Lv'):
                self.page_controller.choose_level(self.page)  # 控制關卡選擇
            self.page = self.page_controller.page_response
            quit_game = self.page_controller.quit
            self.page_controller.choose_menu(self.page)  # 控制 menu 選擇
